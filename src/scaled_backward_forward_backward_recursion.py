...
#   avoid underflow. 
#
#  Let us consider a regime switching VAR process X. Let Z the associated
#  state process. At each time step, we suppose known partial information
#  about Z_t. These information are under the form of the set of possible states
#  denoted by sigma_t  in power set of {0, 1, 2, ..., M-1} where M is the 
#  total number of states. Namely, sigma_t = {0, 4} means that only states
#  0 and 4 are possible at time-step t. 
#
...
#
#  @param Sigma List of T 1D arrays of different lengths, with Sigma[t] in 
#  in power set of {0, 1, 2, ..., M-1} where M is the total number of states.
#  For instance, Sigma[t] = [0, 4] means that only states 0 and 4 are possible 
#  at time-step t.
#  
#  @param A Matrix MxM, A[i,j] = a_{i,j} = P(Z_t=j|Z_{t-1}=i).
#  @param Pi Row vector 1xM, initial state probabilities, Pi[0,k] = P(Z_0 = k).
#
#  @return the log-likelihood of observation data P(X|Sigma).
#  @return the joint probabilities Xi_t = P(Z_t=i, Z_{t+1}=j | X, Sigma).
#  @return the probability to observe the state i at time t 
#   gamma_t = P(z_t=i | X, Sigma).
#  
...
#  @brief
#
#  @return A T length array
#
def compute_prob_sigma(M, Sigma, A, Pi):
    
    #tiny real number used to prevent divison by zero
    tiny_ = np.finfo(0.).tiny
    ...
    #---first time-step
    sigma_0 = Sigma[0]
    prob_sigma[0] = np.sum(Pi[0, sigma_0]) + tiny_
    ...
    for t in range(1, T):
        ...
        tmp_sum = 0.0
        for i in sigma_t_1: 
            tmp_sum = tmp_sum + np.sum(A[i, sigma_t]) 
            
        prob_sigma[t] = tmp_sum + tiny_
...
#
#  @param M The number of states.
#  @param Sigma List of T 1D arrays
#  @param A Matrix MxM
#  @param prob_sigma Array of length T containing probabilities
...
def first_backward_step(M, Sigma, A, prob_sigma):
    ...
    for t in range(T-2, -1, -1): 
        ...
        for z in range(M):     
            ...
            tmp_sum = np.sum( Tau_tilde[t+1, sigma_t_1] * A[z, sigma_t_1] )                
            tau_t_z = tmp_sum / prob_sigma[t]  
...
#  @return 
#   * A matrix TxM Alpha 
#   * T length array C_t, with  C_t[t] = P(X_t=x_t | X_{1-p}^{t-1}, Sigma)
#  
def forward_step(M, LL, Sigma, A, Pi, Tau_tilde, prob_sigma):
    ...
    #---compute C_1
    C_t[0] = np.sum(LL[0, sigma_0] * Pi[0, sigma_0]) + tiny_
    ...
    for z in range(M):      
        ...
        alpha_0_z = aux * \
            (Tau_tilde[0, z] / np.sum(Tau_tilde[0, sigma_0] * Pi[0, sigma_0]))           
    ...
    for t in range(1, T):
        ...
        for j in sigma_t:    
            tmp_sum = tmp_sum + \
            ( LL[t, j] * np.sum(Alpha_tilde[t-1, sigma_t_1] * A[sigma_t_1, j]) )   
        ...
        for z in sigma_t:
            ...
            alpha_t_z = LL[t, z] * \
                    np.sum( Alpha_tilde[t-1, sigma_t_1] * A[sigma_t_1, z] * \
                           (Tau_tilde[t, z] / Tau_tilde[t-1, sigma_t_1]) ) / \
            ...
            #Alpha_tilde[t, z] = 0 if z not in sigma_t
...
def second_backward_step(M, LL, Sigma, A, Pi, Tau_tilde, C_t, prob_sigma):
    ...
    for t in range(T-2, -1, -1):
        ...
        for z in sigma_t:
            ...
            #Beta_tilde[t, z] = 0 if z not in Sigma_t
...
#  @brief 
#  
#  @param Xi This function computes gamma probabilities from Xi probabilities.
#
...
#  @param LL The likelihood matrix of dimension TxM 
#  @param Sigma List of T 1D arrays 
#  @param A Matrix MxM, A[i,j]
#  @param Pi Row vector 1xM
#  @param Tau_tilde Matrix TxM, first backward propagation terms
#  @param Alpha_tilde Matrix TxM, forward propagation terms
